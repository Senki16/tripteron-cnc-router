...
\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/functional_structure.png}
\caption{Functional structure of the machine: the material, energy and information flows
and the elementary functions that transform them.}
\end{figure}
\FloatBarrier
...
\begin{figure}[htbp]\centering
\includegraphics[height=0.78\textheight,keepaspectratio]{figures/morphological_matrix.png}
\caption{Morphological matrix: for every function, the solutions considered and the one
selected.}
\end{figure}
\FloatBarrier

The concept finally built follows this matrix in everything except the transmission: the
matrix selects a ball screw, and what was fitted is a trapezoidal lead screw
(Tr8$\times$8, four starts), which is cheaper and self-supporting in the nut but far less
efficient. Delivery~3 quantifies that choice: \SI{64.1}{\percent} efficiency and no
self-locking.

\subsection{Evaluation matrix}

Each member scored the individual concepts against the technical requirements of the PDS;
the concept with the highest weighted score was taken as the basis of the final one.

\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/evaluation_matrix.png}
\caption{Evaluation matrix of the individual concepts against the technical requirements.}
\end{figure}
\FloatBarrier

The highest score went to the concept of Germ\'an Gabriel Ria\~no (4.5), followed by Juan
Jos\'e Garc\'ia (4.34) and David Amell (4.26). The final concept keeps the frame and axis
layout of the first and the tool mounting of the others.

The evaluation against the PDS selected: stepper motors, power screws, linear guides, a
welded steel frame, a microcontroller-based controller, and the \textbf{Tripteron}
architecture.
