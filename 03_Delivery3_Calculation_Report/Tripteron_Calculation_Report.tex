...
corrected edition can be read side by side with it. The five sections of the original memo
are all here and none was dropped:

\begin{center}\small
\begin{tabular}{@{}ll l@{}}
\toprule
\textbf{Original memo} & \textbf{Pages} & \textbf{This edition} \\
\midrule
1. C\'alculos est\'aticos        & 3--7   & Sheet 2 \\
2. C\'alculos cinem\'aticos      & 8--22  & Sheet 1 \\
3. C\'alculos tornillo de potencia & 23--29 & Sheet 3 \\
4. C\'alculos gu\'ia par R       & 30--37 & Sheet 3 (its screw content) + Sheet 4 (the guides it never checked) \\
5. C\'alculos chasis, MEF        & 38--60 & Sheet 5, with its original plots \\
\bottomrule
\end{tabular}
\end{center}

Kinematics is placed first because the static analysis needs the poses it produces; in the
original memo the two appear in the opposite order.
...
\section{What the original sheet contained}\label{sec:gui-orig}

Nothing of the original \emph{Gu\'ias par R} sheet is dropped; it is relocated. That sheet
set out to determine, in its own words, the raising and lowering torque, the total torque,
the efficiency, the shear stress in torsion and the buckling --- all of them properties of
the \emph{lead screw}, not of the guides, and all of them already computed in Sheet~3.
The table below says where each of its objectives is answered and with what value.

\begin{center}\small
\begin{tabular}{@{}l>{\raggedright\arraybackslash}p{4.6cm}l l@{}}
\toprule
\textbf{Original objective} & \textbf{Answered in} & \textbf{Original value} & \textbf{Corrected value} \\
\midrule
Raising torque $T_s$        & Sheet 3, eq.~\eqref{eq:TR} & \SI{302.72}{\newton\milli\metre} & \SI{311.9}{\newton\milli\metre} \\
Lowering torque $T_d$       & Sheet 3, eq.~\eqref{eq:TL} & \SI{-94.39}{\newton\milli\metre} & \SI{-111.9}{\newton\milli\metre} \\
Collar torque $T_c$         & Sheet 3                    & \SI{27.342}{\newton\milli\metre} & \SI{9.7}{\newton\milli\metre} \\
Total raising torque        & Sheet 3                    & \SI{311.838}{\newton\milli\metre} & \SI{321.6}{\newton\milli\metre} \\
Total lowering torque       & Sheet 3                    & \SI{-67.048}{\newton\milli\metre} & \SI{-102.2}{\newton\milli\metre} \\
Efficiency                  & Sheet 3, eq.~\eqref{eq:eff} & 1.948 & \SI{64.1}{\percent} \\
Torsional shear             & Sheet 3                    & \SI{4.63}{\mega\pascal} & \SI{7.58}{\mega\pascal} \\
Thread shear                & Sheet 3                    & \SI{2379.96}{\mega\pascal} & \SI{3.77}{\mega\pascal} \\
Axial stress                & Sheet 3                    & \SI{72.21}{\mega\pascal} & \SI{5.73}{\mega\pascal} \\
Buckling $P_{cr}$           & Sheet 3                    & \SI{9428.8}{\newton} & \SI{519.9}{\newton} \\
Self-locking                & Sheet 3                    & not satisfied & not satisfied (confirmed) \\
\bottomrule
\end{tabular}
\end{center}

What follows is what that sheet was named after and never did: the verification of the
guides themselves.
...
work table. The frame is the one component whose behaviour cannot be captured by a hand
calculation: it is a three-dimensional redundant structure, so it was verified by finite
elements in SolidWorks Simulation.

This sheet keeps the original study in full --- its model, its loads, its nine result plots
and its conclusions --- and adds to it the post-processing of the exported nodal data, so
that every number quoted here is reproducible from the files in the repository rather than
read off a screenshot. The raw exports live in
\texttt{04\_Calculations/data/fea\_exports/} and are processed by
\texttt{tripteron/fea\_postprocess.py}.

\section{Objective of the calculation}\label{sec:fea-obj}

\begin{enumerate}[leftmargin=*,nosep]
  \item Verify that the frame stays in the elastic range under the weight of the three
        axis assemblies and the reactions of Sheet~2.
...
\section{Variables of the calculation model}\label{sec:fea-var}
...
\textbf{Symbol} & \textbf{Description} & \textbf{Value} \\
\midrule
$F_1$ & Total weight carried by axis 1 & \SI{9.762}{\kilo\gram} = \SI{95.75}{\newton} \\
$F_2$ & Total weight carried by axis 2 & \SI{9.762}{\kilo\gram} = \SI{95.75}{\newton} \\
$F_3$ & Total weight carried by axis 3 & \SI{9.762}{\kilo\gram} = \SI{95.75}{\newton} \\
$P_m$ & Mass of one NEMA 34 motor      & \SI{5.216}{\kilo\gram} \\
$P_t$ & Mass of one lead screw         & \SI{1.120}{\kilo\gram} \\
$P_g$ & Mass of one linear guide       & \SI{2.530}{\kilo\gram} \\
$P_h$ & Mass of one arm                & \SI{0.322}{\kilo\gram} \\
$P_e$ & Mass of the end effector       & \SI{0.574}{\kilo\gram} \\
\bottomrule
\end{tabular}
\end{center}

All masses were read from the SolidWorks physical-properties tool; each axis load is the
sum of the components that rest on that beam, so they carry a small approximation error.

\section{Image of the calculation model}\label{sec:fea-img}

\begin{figure}[htbp]\centering
\begin{minipage}[t]{0.47\textwidth}\centering
\includegraphics[width=\textwidth]{figures/fea_model_loads.png}\\[3pt]
{\footnotesize Applied forces and fixed supports}
\end{minipage}\hfill
\begin{minipage}[t]{0.47\textwidth}\centering
\includegraphics[width=\textwidth]{figures/fea_model_weights.png}\\[3pt]
{\footnotesize Components whose weight loads the frame}
\end{minipage}
\caption{Calculation model: the four fixed feet, the three axis loads and the components
whose weight produces them.}
\end{figure}
\FloatBarrier

\section{Values and calculation model}\label{sec:fea-model}

\begin{center}
\begin{tabular}{@{}l>{\raggedright\arraybackslash}p{10.6cm}@{}}
\toprule
Geometry          & Chassis CAD at full size, welded tubular frame \\
Element type      & Beam elements \\
Material          & Alloy steel, $E=\SI{210}{\giga\pascal}$, $S_y=\SI{620}{\mega\pascal}$ \\
Interaction       & Global interaction, rigid joints, independent meshing \\
Fixation          & Four fixed restraints, one per foot \\
External loads    & Three forces of \SI{95.75}{\newton} on their respective beams \\
Mesh              & Curvature-based mesh, standard density \\
Studies           & Linear static analysis and linear (eigenvalue) buckling analysis \\
Post-processing   & Nodal values read with \emph{identify values} on the loaded faces and
                    exported to CSV \\
\bottomrule
\end{tabular}
\end{center}

Beam elements are the right idealisation here: the frame is made of slender tubes loaded
mainly in bending and axial force and welded at the nodes, and a beam model reproduces
that behaviour with a fraction of the degrees of freedom of a solid mesh. Local weld
stresses are not captured and are covered by the weld sizing of Delivery~2.

\subsection{Scope of the load case}

The load applied in the finite element model is the \emph{self weight} of the three axis
assemblies, \SI{95.75}{\newton} each. It does not include the cutting force, whose
reactions were only obtained later, in Sheet~2. Those reactions reach
\SI{162}{\newton} on the vertical axis, i.e.\ \SI{69}{\percent} more than the weight
already applied on that beam. Because the static safety factor of the frame is 66, adding
them cannot bring the structure anywhere near its limit; the deflection, however, would
grow roughly in proportion, so the \SI{0.102}{\milli\metre} below should be read as the
figure for a machine at rest and about \SI{0.17}{\milli\metre} while cutting.

\section{Results of the calculation model}\label{sec:fea-res}

\subsection{Static analysis}

\begin{figure}[htbp]\centering
\begin{minipage}[t]{0.47\textwidth}\centering
\includegraphics[width=\textwidth]{figures/fea_bending.png}\\[2pt]{\footnotesize Bending stress [MPa]}
\end{minipage}\hfill
\begin{minipage}[t]{0.47\textwidth}\centering
\includegraphics[width=\textwidth]{figures/fea_axial.png}\\[2pt]{\footnotesize Axial stress [MPa]}
\end{minipage}

\vspace{8pt}
\begin{minipage}[t]{0.47\textwidth}\centering
\includegraphics[width=\textwidth]{figures/fea_shear1.png}\\[2pt]{\footnotesize Shear stress, direction 1 [MPa]}
\end{minipage}\hfill
\begin{minipage}[t]{0.47\textwidth}\centering
\includegraphics[width=\textwidth]{figures/fea_shear2.png}\\[2pt]{\footnotesize Shear stress, direction 2 [MPa]}
\end{minipage}

\vspace{8pt}
\begin{minipage}[t]{0.47\textwidth}\centering
\includegraphics[width=\textwidth]{figures/fea_strain.png}\\[2pt]{\footnotesize Strain [--]}
\end{minipage}\hfill
\begin{minipage}[t]{0.47\textwidth}\centering
\includegraphics[width=\textwidth]{figures/fea_displacement.png}\\[2pt]{\footnotesize Resultant displacement [mm]}
\end{minipage}
\caption{Static analysis of the chassis: stress components, strain and displacement.}
\end{figure}
\FloatBarrier
...
\includegraphics[width=\textwidth]{figures/fea_buckling_x.png}\\[2pt]{\footnotesize Mode 1, amplitude $x$ [mm]}
\end{minipage}\hfill
\begin{minipage}[t]{0.32\textwidth}\centering
\includegraphics[width=\textwidth]{figures/fea_buckling_y.png}\\[2pt]{\footnotesize Mode 1, amplitude $y$ [mm]}
\end{minipage}\hfill
\begin{minipage}[t]{0.32\textwidth}\centering
\includegraphics[width=\textwidth]{figures/fea_buckling_z.png}\\[2pt]{\footnotesize Mode 1, amplitude $z$ [mm]}
\end{minipage}
\caption{First buckling mode of the frame: a lateral sway of the uprights.}
\end{figure}
\FloatBarrier

\subsection{Exported values}

The nodal values read on the loaded faces were exported and post-processed by
\texttt{fea\_postprocess.py}; the table below replaces the nine value tables of the
original study with a single one, and the figure replaces its nine graphs.
...
\caption{Range of each exported quantity and the resulting margins.}
\end{figure}
\FloatBarrier

\section{Results and conclusions of the calculation model}\label{sec:fea-conc}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Stiffness is the meaningful result.} The maximum displacement of
        ...
        the intended use (light milling of aluminium and plastics), but it is one of the
        two numbers that limit the achievable accuracy of the machine --- the other being
        the \SI{0.154}{\milli\metre} of the guide rods in Sheet~4, which is larger.
  ...
  \item \textbf{The load case can be tightened.} Rerunning the study with the joint
        reactions of Sheet~2 instead of the bare self weight would not change any
        conclusion, but it would give a deflection figure valid while cutting. That is the
        one change worth making to this study.
\end{enumerate}
